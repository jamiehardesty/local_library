...
function getBooksBorrowedCount(books) {
  return (borrowed = books.filter(({ borrows }) => !borrows[0].returned)
    .length); // [0] is necessary !!
}

function getMostCommonGenres(books) {// try using .reduce()
  ...
  for (let book in books) {
    ...
    if (firstRun === false) {
      arr.push({ name: books[book].genre, count: 1 });
    }
  }
...
function getMostPopularAuthors(books, authors) {
  let arr = []; // this establishes the shape
  for (let book in books) {
    let authMatch = authors.find((auth) => auth.id === books[book].authorId);
